...
class EstimateSwapOutput:
    ...
    def get_swap_output(
        ...
        if in_token_address is None:
            in_token_address = OrderArgumentParser(config=self.config).find_key_by_symbol(self.tokens, in_token_symbol)
        if out_token_address is None:
            out_token_address = OrderArgumentParser(config=self.config).find_key_by_symbol(
                self.tokens, out_token_symbol
            )
        if token_amount_expanded is None:
            token_amount_expanded = token_amount * 10 ** self.tokens[in_token_address]["decimals"]

        swap_route = determine_swap_route(self.markets, in_token_address, out_token_address)
        output = self.estimated_swap_output(self.markets[swap_route[0][0]], in_token_address, token_amount_expanded)
    ...
    def estimated_swap_output(self, market: dict, in_token: str, token_amount_expanded: int):
        """
        ...
        estimated_swap_output_parameters = {
            ...
            "market_addresses": [market["gmx_market_address"]] + token_addresses,
